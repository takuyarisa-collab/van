 ...
 * game ロジックとは分離。scene ごとに opts で薄さやシードを調整可能。
 */
 ...
 * @param {number} [opts.seed] レイアウト再現用
 * @param {number} [opts.depthBase] 奥の深度（より小さい＝背面）
 * @param {number} [opts.gradientTop]
 * @param {number} [opts.gradientBottom]
 * @param {number} [opts.structureAlpha] グリッドの不透明度係数（0〜1）
 * @param {number} [opts.noiseAlpha] 粒子・擦れの不透明度係数
 * @param {number} [opts.fragmentAlpha] HUD 断片の不透明度係数
 * @param {number} [opts.noiseDots] 粒子の個数上限
 */
